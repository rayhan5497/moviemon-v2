...
const allowedSorts = filtersData['tv-sorts'].map((t) => t.value);
...
export default function FilterMovies() {
  ...
  useEffect(() => {
    ...
    const invalid = !allowedSorts.includes(urlParam) || extraSegments;
  ...
  // ----------------------
  useEffect(() => {
    const zone = searchParams.get('timezone') || 'America/New_York';
  ...
  const handleSortChange = (e) => {
    ...
    // We only modify the type part of the URL
    navigate(`/tv/${clickedSort}?${nextSearchParams.toString()}`);
  };

  const handleZoneChange = (e) => {
    const clickedZone = e.target.value || '';
    setSelectedZone(clickedZone);

    // Build new search params based on current searchParams
    const next = new URLSearchParams(searchParams);
    if (clickedZone) next.set('timezone', clickedZone);
  ...
  useEffect(() => {
    const onDocClick = (e) => {
      ...
      const clickedInsideRegion = zoneElRef.current?.contains(target);
      ...
      if (
        ...
        setOpen({ media: false, sort: false, timezone: false });
      }
    };
  ...
  const sortLabel =
    filtersData['tv-sorts'].find((t) => t.value === selectedSort)?.label || '';
  const zoneLabel = selectedZone
    ? filtersData.timezones.find((r) => r.value === selectedZone)?.label || ''
    : '';
  ...
  return (
    <>
      ...
      {isAnyOpen && (
        <Backdrop
          onClick={() => setOpen({ type: false, region: false })}
      ...
      <div className="filters bg-primary text-primary p-2 pt-0 px-3 md:px-4 rounded-lg space-y-4 sticky top-0 z-40">
          ...
          <span className="font-light">
            {!showFilter && (
              <>
                ...
                {zoneLabel && <span className="mr-1">🔹{zoneLabel}</span>}
              </>
            )}
          </span>
        </h1>
        ...
        >
          ...
          <FilterSection
            className={`timezones filter-category ${
              isFiltersOpen('timezone') ? 'md:w-full' : ''
            }`}
            ref={zoneElRef}
            onClick={() => handleFiltersToggle('timezone')}
